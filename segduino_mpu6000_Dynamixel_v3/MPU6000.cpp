...
/* 
   ...
   variants however
 */
const float MPU6000::_accel_scale = 9.81 / 4096.0;

/* pch: I believe the accel and gyro indicies are correct
 *      but somone else should please confirm.
 */
const uint8_t MPU6000::_gyro_data_index[3]  = { 5, 4, 6 };
const int8_t  MPU6000::_gyro_data_sign[3]   = { 1, 1, -1 };

const uint8_t MPU6000::_accel_data_index[3] = { 1, 0, 2 };
const int8_t  MPU6000::_accel_data_sign[3]  = { 1, 1, -1 };

const uint8_t MPU6000::_temp_data_index = 3;
...
bool MPU6000::_dmp_initialised = false;
uint8_t MPU6000::_received_packet[DMP_FIFO_BUFFER_SIZE];    // FIFO packet buffer
uint8_t MPU6000::_fifoCountH;			            // high byte of number of elements in fifo buffer
uint8_t MPU6000::_fifoCountL;			            // low byte of number of elements in fifo buffer
Quaternion MPU6000::quaternion;			            // holds the 4 quaternions representing attitude taken directly from the DMP

MPU6000::MPU6000 ( )
{
  _cs_pin = 53; /* chip select is pin 53 */
  _gyro.x = 0;
  _gyro.y = 0;
  _gyro.z = 0;
  _accel.x = 0;
...
bool MPU6000::new_data_available( void )
{
    return _count != 0;
}

float MPU6000::gx() { return _gyro.x; }
float MPU6000::gy() { return _gyro.y; }
float MPU6000::gz() { return _gyro.z; }

void MPU6000::get_gyros( float * g )
{
  g[0] = _gyro.x;
  g[1] = _gyro.y;     
  g[2] = _gyro.z;
...
float MPU6000::ax() { return _accel.x; }
float MPU6000::ay() { return _accel.y; }
float MPU6000::az() { return _accel.z; }

void MPU6000::get_accels( float * a )
{
  a[0] = _accel.x;
  a[1] = _accel.y;
  a[2] = _accel.z;
...
void MPU6000::get_sensors( float * sensors )
{
  sensors[0] = _gyro.x;
  sensors[1] = _gyro.y;
  sensors[2] = _gyro.z;
...
void MPU6000::getMotion6 (int16_t* ax, int16_t* ay, int16_t* az, int16_t* gx, int16_t* gy, int16_t* gz) {
  // TO DO
}

float MPU6000::temperature() { return _temp; }

uint32_t MPU6000::sample_time()
{
  uint32_t us = micros();
...
void MPU6000::reset_sample_time()
{
...
void MPU6000::data_interrupt(void)
{
...
float MPU6000::_temp_to_celsius ( uint16_t regval )
{
    /* TODO */
    return 20.0;
}
...
float MPU6000::get_gyro_drift_rate(void)
{
...
void MPU6000::set_gyro_offsets_scaled(float offX, float offY, float offZ)
{
...
void MPU6000::set_accel_offsets_scaled(float offX, float offY, float offZ)
{
...
void MPU6000::set_accel_offsets(int16_t offsetX, int16_t offsetY, int16_t offsetZ)
{
	int aux_int;
	uint8_t regs[2];
...
void MPU6000::dmp_register_write(uint8_t bank, uint8_t address, uint8_t num_bytes, uint8_t data[])
{
  uint8_t dump;
...
void MPU6000::dmp_set_gyro_calibration()
{
	uint8_t regs[4];
	regs[0]=0x4C;
	regs[1]=0xCD;
...
void MPU6000::dmp_set_accel_calibration()
{
	uint8_t regs[6];
	regs[0]=0x00;
	regs[1]=0x00;
...
void MPU6000::dmp_apply_endian_accel()
{
	uint8_t regs[4];
	regs[0]=0x00;
	regs[1]=0x00;
...
void MPU6000::dmp_set_mpu_sensors()
{
	uint8_t regs[6];
	regs[0]=0x0C;
	regs[1]=0xC9;
...
void MPU6000::dmp_set_bias_from_no_motion()
{
	uint8_t regs[4];
	regs[0]=0x0D;
	regs[1]=0x35;
...
void MPU6000::dmp_set_bias_none()
{
	uint8_t regs[4];
	regs[0]=0x98;
	regs[1]=0x98;
...
// dmp_set_fifo_interrupt
void MPU6000::dmp_set_fifo_interrupt()
{
	uint8_t regs[1];
	regs[0]=0xFE;
...
void MPU6000::dmp_send_quaternion()
{
	uint8_t regs[5];
	regs[0]=0xF1;
	regs[1]=0x20;
...
void MPU6000::dmp_set_fifo_rate(uint8_t rate)
{
	uint8_t regs[2];
	regs[0]=0x00;
	regs[1]=rate;
...
void MPU6000::dmp_set_sensor_fusion_accel_gain(uint8_t gain)
{
	uint8_t dump;
	//inv_key_0_96
...
///////////////////////////////////////////////////////////////////////////////////////
//                              TIMER 2 HELPER FUNCTIONS                             //
///////////////////////////////////////////////////////////////////////////////////////

void MPU6000::timer2_start() {
  TIMSK2 |= (1<<TOIE2);
}

void MPU6000::timer2_stop() {
  TIMSK2 &= ~(1<<TOIE2);
}

void MPU6000::timer2_init () {
  // Configure timer2 in normal mode (pure counting, no PWM etc.)   
  TCCR2A &= ~((1<<WGM21) | (1<<WGM20));   
  TCCR2B &= ~(1<<WGM22);   
  // Select clock source: internal I/O clock   
  ASSR &= ~(1<<AS2);   
  // Disable Compare Match A interrupt (only want overflow)  
  TIMSK2 &= ~(1<<OCIE2A);   
